...
render () {
  if (this.props.loaded === LOADING) return <span>loading items for {this.props.classificationId}</span>
  if (this.props.loaded === FAILED) return <span>Failed loading results for {this.props.classificationId}</span>
  ...
  if(this.state.selectedItem === ''){
    ...
    return (
      <span>
        ...
        <select onChange= {this.handleChange} value={this.state.selectedItem} ref="refSelectItem">
...
console.log('un selectedItem : -' + this.state.selectedItem + '-');
    ...
    <span>
      ...
      <br/>
      <ItemCorrespondences item={this.state.selectedItem} classification={this.props.classificationId} hash={hash}/>
    </span>
    )
  }
